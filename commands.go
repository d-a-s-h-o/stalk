...
var (
	...
	SecretCMDs = []CMD{
		...
		{"cowsay", cowsayCMD, "`message`", "Send ascii art of a cow saying a message"},
...
func init() {
	StartupTime = time.Now() // Initialize StartupTime

	// add Matt Gleich's blackbird theme from https://github.com/blackbirdtheme/vscode/blob/master/themes/blackbird-midnight-color-theme.json#L175
...
func themeCMD(line string, u *User) {
	...
	u.room.broadcast(Devbot, "Themes do not currently work because TALK is switching to using glamour for rendering.")
	if line == "list" {
...
* \u: your user name
* \h, \H: chat colored like your username
...
func unameCMD(rest string, u *User) {
	if unameCommit == "" || unameTime == "" {
		u.room.broadcast("", "No uname output available. Build TALK with `"+color.HiYellowString(`go build -ldflags "-X 'main.unameCommit=$(git rev-parse HEAD)' -X 'main.unameTime=$(date)'"`)+"` to enable.")
		return
	}
	u.room.broadcast("", "TALK - A SSH Chat ("+unameCommit+") "+unameTime)
}
...
func echoCMD(rest string, u *User) {
	...
	u.room.broadcastNoBridges("", rest)
}

func cowsayCMD(rest string, u *User) {
	if rest == "" {
		u.room.broadcast(Devbot, "What do you want the cow to say?")
		return
	} else if len(rest) > 40 {
		u.room.broadcast(Devbot, "That's too long for the cow to say")
		return
	} else if strings.Contains(rest, "\n") {
		u.room.broadcast(Devbot, "The cow can't say that")
		return
	}
	cow := `  _______\n< ` + rest + ` >\n  -------\n         \   ^__^\n          \  (oo)\_______\n             (__)\       )\/\n                 ||----w |\n                 ||     ||`
	u.room.broadcast("", cow)
}
