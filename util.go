...
func keepSessionAlive(s ssh.Session) {
	for {
		...
		_, err := s.SendRequest("keepalive@chat", true, nil)
		if err != nil {
			return
		}
	}
...
func devbotChat(room *Room, line string) {
	...
	if strings.Contains(line, "where") && strings.Contains(line, "repo") {
		devbotRespond(room, []string{"The repo's at github.com/d-a-s-h-o/talk!", ":star: github.com/d-a-s-h-o/talk :star:", "# github.com/d-a-s-h-o/talk"}, 100)
	}
